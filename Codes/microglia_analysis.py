...
week0_ck = pd.read_csv(expression_week0_ck, sep='\t',index_col=0)
week0_ckp25 = pd.read_csv(expression_week0_ckp25, sep='\t',index_col=0)
week1_ck = pd.read_csv(expression_week1_ck, sep='\t',index_col=0)
week1_ckp25 = pd.read_csv(expression_week1_ckp25, sep='\t',index_col=0)
week2_ck = pd.read_csv(expression_week2_ck, sep='\t',index_col=0)
week2_ckp25 = pd.read_csv(expression_week2_ckp25, sep='\t',index_col=0)
week6_ck = pd.read_csv(expression_week6_ck, sep='\t',index_col=0)
week6_ckp25 = pd.read_csv(expression_week6_ckp25, sep='\t',index_col=0)
...
def perform_regression(gene1, gene2,regression_function):
    ...
    if gene1 not in week0_ck.index:
      avg_gene1_week0_ck=0
    else:
      avg_gene1_week0_ck = week0_ck.loc[gene1].mean()
    if gene1 not in week0_ckp25.index:
      avg_gene1_week0_ckp25=0
    else:
      avg_gene1_week0_ckp25 = week0_ckp25.loc[gene1].mean()
    if gene1 not in week1_ck.index:
      avg_gene1_week1_ck=0
    else:
      avg_gene1_week1_ck = week1_ck.loc[gene1].mean()
    if gene1 not in week1_ckp25.index:
      avg_gene1_week1_ckp25=0
    else:
      avg_gene1_week1_ckp25 = week1_ckp25.loc[gene1].mean()
    if gene1 not in week2_ck.index:
      avg_gene1_week2_ck=0
    else:
      avg_gene1_week2_ck = week2_ck.loc[gene1].mean()
    if gene1 not in week2_ckp25.index:
      avg_gene1_week2_ckp25=0
    else:
      avg_gene1_week2_ckp25 = week2_ckp25.loc[gene1].mean()
    if gene1 not in week6_ck.index:
      avg_gene1_week6_ck=0
    else:
      avg_gene1_week6_ck = week6_ck.loc[gene1].mean()
    if gene1 not in week6_ckp25.index:
      avg_gene1_week6_ckp25=0
    else:
      avg_gene1_week6_ckp25 = week6_ckp25.loc[gene1].mean()
    if gene2 not in week0_ck.index:
      avg_gene2_week0_ck=0
    else:
      avg_gene2_week0_ck = week0_ck.loc[gene2].mean()
    if gene2 not in week0_ckp25.index:
      avg_gene2_week0_ckp25=0
    else:
      avg_gene2_week0_ckp25 = week0_ckp25.loc[gene2].mean()
    if gene2 not in week1_ck.index:
      avg_gene2_week1_ck=0
    else:
      avg_gene2_week1_ck = week1_ck.loc[gene2].mean()
    if gene2 not in week1_ckp25.index:
      avg_gene2_week1_ckp25=0
    else:
      avg_gene2_week1_ckp25 = week1_ckp25.loc[gene2].mean()
    if gene2 not in week2_ck.index:
      avg_gene2_week2_ck=0
    else:
      avg_gene2_week2_ck = week2_ck.loc[gene2].mean()
    if gene2 not in week2_ckp25.index:
      avg_gene2_week2_ckp25=0
    else:
      avg_gene2_week2_ckp25 = week2_ckp25.loc[gene2].mean()
    if gene2 not in week6_ck.index:
      avg_gene2_week6_ck=0
    else:
      avg_gene2_week6_ck = week6_ck.loc[gene2].mean()
    if gene2 not in week6_ckp25.index:
      avg_gene2_week6_ckp25=0
    else:
      avg_gene2_week6_ckp25 = week6_ckp25.loc[gene2].mean()



    # Calculate the relative expression changes
    x = [avg_gene1_week0_ckp25,avg_gene1_week0_ck,avg_gene1_week1_ckp25,avg_gene1_week1_ck,avg_gene1_week2_ckp25,avg_gene1_week2_ck,avg_gene1_week6_ckp25,avg_gene1_week6_ck]
    y = [avg_gene2_week0_ckp25,avg_gene2_week0_ck,avg_gene2_week1_ckp25,avg_gene2_week1_ck,avg_gene2_week2_ckp25,avg_gene2_week2_ck,avg_gene2_week6_ckp25,avg_gene2_week6_ck]
