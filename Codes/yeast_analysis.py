...
sacc_before = pd.read_csv(expression_before_sacc, sep='\t')
sacc_after = pd.read_csv(expression_after_sacc, sep='\t')
sacc_before=sacc_before.T
sacc_after=sacc_after.T
...
def perform_regression(gene1, gene2,regression_function):
    ...
    if gene1 not in sacc_before.index:
      avg_gene1_sacc_before=0
    else:
      avg_gene1_sacc_before = sacc_before.loc[gene1].mean()
    if gene1 not in sacc_after.index:
      avg_gene1_sacc_after=0
    else:
      avg_gene1_sacc_after = sacc_after.loc[gene1].mean()

    if gene2 not in sacc_before.index:
      avg_gene2_sacc_before=0
    else:
      avg_gene2_sacc_before = sacc_before.loc[gene2].mean()
    if gene2 not in sacc_after.index:
      avg_gene2_sacc_after=0
    else:
      avg_gene2_sacc_after = sacc_after.loc[gene2].mean()


    # Calculate the relative expression changes
    x = [avg_gene1_sacc_before,avg_gene1_sacc_after]
    y = [avg_gene2_sacc_before,avg_gene2_sacc_after]
    # Perform regression
...
file1 = pd.read_csv(expression1_path, sep='\t')
file2 = pd.read_csv(expression2_path, sep='\t')

file1=file1.T
file2=file2.T

all_genes = list(set(file1.index).union(file2.index))
# print (all_genes.index('YDL248W'))
